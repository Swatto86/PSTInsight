//! Block data de-obfuscation.
//!
//! PST data blocks may be obfuscated with one of three methods recorded in the
//! header (`bCryptMethod`): none (0), permute / "compressible" (1), or cyclic
//! (2). Only none and permute are supported here.
//!
//! NOTE: the substitution table below is the canonical compressible-encoding
//! table reproduced across the open-source ecosystem (libpst, etc.). It is
//! transcribed here and SHOULD be verified against `[MS-PST] 5.1` or a known
//! reference if you observe garbled text — a single wrong byte silently
//! corrupts decoded output. The unit test asserts it is a valid permutation,
//! which catches transcription errors but not value swaps.

use crate::error::{Error, Result};

/// Maps a stored (obfuscated) byte to its plaintext byte for the permute method.
const DECODE_PERMUTE: [u8; 256] = [
...
/// Decode a leaf data block in place according to the header crypt method.
///
/// `_bid` is unused for the supported methods but is part of the cyclic key.
pub fn decode(method: u8, _bid: u64, data: &mut [u8]) -> Result<()> {
    match method {
        0 => Ok(()),
        1 => {
            for b in data.iter_mut() {
                *b = DECODE_PERMUTE[*b as usize];
            }
            Ok(())
        }
        m => Err(Error::UnsupportedEncryption(m)),
    }
}

#[cfg(test)]
mod tests {
    use super::DECODE_PERMUTE;

    #[test]
    fn table_is_a_permutation() {
        let mut seen = [false; 256];
        for &v in DECODE_PERMUTE.iter() {
            assert!(!seen[v as usize], "duplicate value {v} in permute table");
            seen[v as usize] = true;
        }
        assert!(seen.iter().all(|&s| s), "permute table is not a bijection");
    }
}
